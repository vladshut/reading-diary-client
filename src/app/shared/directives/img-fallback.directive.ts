/**
 * Created by vshut on 28/01/2020.
 */
...
export class ImgFallbackDirective implements OnDestroy {
  ...
  imgFallbackSrc = '/assets/img/loading.gif';
  ...
  private onError() {
    if (this.nativeElement.getAttribute('src') !== this.imgFallbackSrc) {
      this.isApplied = true;
      this.renderer.setAttribute(this.nativeElement, 'src', this.imgFallbackSrc);
    } else {
      this.removeOnLoadEvent();
    }
  }
