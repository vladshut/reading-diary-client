...
@Component({
  ...
  styleUrls: ['./followees-page.component.css']
})
export class FolloweesPageComponent extends WithLoading() implements OnInit {
  ...
  private loadUsers(filter: {page: number}) {
    const $users = this.userService.followees(this.currentUser.id, filter);
